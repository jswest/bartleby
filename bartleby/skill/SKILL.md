...
`save_finding --title <t> --description <d> --body-file <path>` | Persist a research finding. Keep the title a short headline, not a sentence — the detail belongs in `--description` and the body. Body comes from a `--body-file` staged under `~/.bartleby/tmp/` (see "Where to stage scratch"); citations are the `[^chunk:N]` markers in the prose, and at least one is required.
| `edit_finding --finding-id finding:<id> ...` | Update an existing finding in place (title / description / body). Use it to fix malformed citations or revise — don't fork a new finding (`merge_findings` collapses ones that already fragmented). |
