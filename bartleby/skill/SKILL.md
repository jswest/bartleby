...
**Verify the chunk supports the claim.** `save_finding` checks that every `[^chunk:N]` chunk_id *exists* in the project. It does not — and cannot — verify that the chunk *supports the claim* you're attaching it to. That part is on you. Two failure modes to avoid:
