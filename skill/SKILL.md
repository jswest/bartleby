...
bartleby skill edit_finding --finding 12 --body-file ~/.bartleby/tmp/finding.md
bartleby skill edit_finding --finding 12 --title "Updated title"
bartleby skill merge_findings --from 20,21,22 --into 23 --body-file ~/.bartleby/tmp/merged.md  # fold dups into one, delete sources
bartleby skill delete_finding --finding 7              # retract a stale finding (chunks + citations removed)
...
bartleby skill read_finding --finding 12              # the whole finding: body, chunk_ids, resolved citations
...
| `merge_tags --from <a> --into <b>` | Move all assignments from `--from` onto `--into`, then delete `--from`. Pre-existing duplicates collapse cleanly. |
