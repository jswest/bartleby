...
A session with `memory_enabled = 0` must not see *other sessions'* findings — enforced at the **script level**, not the prompt level. Two enforcement shapes: `skill_scripts/search.py` silently excludes **all** findings from results regardless of flags (ranked retrieval is the contamination vector — don't soften this); the direct-read commands `read_finding` / `list_findings` instead narrow to the session's **own** findings (a run can read back what it wrote, never another session's). `edit_finding` gates the same way — it echoes the stored body in its response, so an ungated edit on a foreign finding is a read-by-write bypass (GH-0272). `save_finding` stays open so a memory-off run can still produce findings for later comparison; the pure mutators `delete_finding` / `merge_findings` likewise stay open (they disclose no foreign body — GH-0056). The line: a command gates iff its response discloses content the caller didn't supply.
