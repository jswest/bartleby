...
- **Agent-facing id *values* are type-tagged (issue #624, supersedes the prompt-only #623)**: where #573 names the flag, #624 types the value. Every entity id a skill script emits to stdout is rendered `<type>:<id>` — `chunk:`/`document:`/`finding:`/`image:`/`tag:` (plus `summary:` for a chunk's polymorphic `source_id`) — and every `--*-id` flag and the inline citation marker accept **only** that form; a bare or wrong-typed id (e.g. a `document:` value to `--chunks`) is rejected (`USAGE_ERROR`), never silently resolved against the colliding row. This is **interface-layer only** — parse on input / format on output via `bartleby/skill_scripts/_ids.py` (`parse_id`, the `prefixed_int`/`prefixed_int_list` argparse factories, and `format_output_ids`, a fixed field→type map; `source_id` is prefixed by `source_kind` at its sites, not via the map) — so **storage stays integer**: `finding_citations` rows are bare ints, no `SCHEMA_VERSION` bump, no re-ingest. The citation marker is now `[^chunk:N]` (was `[^N]`), slotting into the existing `[^<scheme>:<ref>]` grammar that also carries external `[^url:…]`/`[^doc:…]`; `[^document:N]` → `WRONG_CITATION_TYPE` and bare `[^N]`/`[N]` → `MALFORMED_CITATION`. `[^finding:N]` is an accepted **unidirectional finding→finding link** (issue #654): validated at write (target must exist, else `UNKNOWN_FINDING_LINKS`), surfaced by `read_finding` as `dangling_finding_links` when a target later vanishes, and rendered as a link in the finding view — no table, storage stays text. This kills the `chunk_id`/`document_id` namespace collision (independent INTEGER PKs with overlapping ranges) structurally. Provenance — whether a correctly-typed citation actually *supports* the claim — is explicitly out of scope (a separate future issue).
