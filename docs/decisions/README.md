...
- [GH-0297 — omnibus issues carry a comment-anchored `omnibus-checklist` block (one `- [ ] #<N>` line per sub-issue, kept beside the prose narrative); `/ship` seeds it on first ship, ticks by `#<N>` match on sub-PR-open, and reads it as the promotion `Closes` manifest; the format layer that #265's serialized reconciler builds on (issue #297)](GH-0297-checkbox-regime-omnibus-0001.md)
